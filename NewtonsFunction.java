...
public class NewtonsFunction implements ODEFunctionInterface {
	...
	public RateInterface call(double t, StateInterface y) {
		...
		for (int i = 0; i < castedY.celestialBodies.size()-1; i++) {
			totalForce = new Vector(0, 0, 0);

			// calculating the total force for each body in the system except for the rocket, depends on all other
			// bodies
		...
		//Calculate the total acceleration of the rocket seperately, since it needs to 
		//consider both Newton's law of Gravitation from the other bodies as well
		//as the acceleration added from the thrust, solved in the FuelSolver class
		
		
		totalForce = new Vector(0, 0, 0);
		body1 = castedY.celestialBodies.get(11);
		
			for (int j = 0; j < castedY.celestialBodies.size()-1; j++) {
				// finding the distance between two bodies
				body2 = castedY.celestialBodies.get(j);
				Vector distance = (Vector) body2.getLocation().sub(body1.getLocation());
			...
			Vector gravAccel = totalForce.divide(body1.getMass());
			
			//System.out.println(gravAccel.toString());
			
			
			//Mdot > 0 means the rocket wants to thrust, meaning we calculate the thrust & fuel
			if (body1.getMdot() > 0){
				Vector solved = fuel.solve(body1.getMdot());
				
				//Add the thruster's acceleration to the gravitational acceleration
				gravAccel = (Vector) gravAccel.add(solved);
			}
			
			/*
				System.out.println("Thrust calculated");
				System.out.println(gravAccel.toString());
				System.out.println();
			*/
