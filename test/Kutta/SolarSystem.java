...
public class SolarSystem {
	...
	public void update(){
		for(int i=1; i < bodies.size(); i++) {
			...
			Vector k2 = (Vector) bodies.get(i).getAcceleration().mul(30).add(k1).add(k1.divide(2));
			Vector k3 = (Vector) bodies.get(i).getAcceleration().mul(30).add(k1).add(k2.divide(2));
			Vector k4 = (Vector) bodies.get(i).getAcceleration().mul(60).add(k1).add(k3);


			Vector change = (Vector) k1.add(k2.mul(2)).add(k3.mul(2)).add(k4);
			change = (Vector) change.divide(6).mul(30);
			bodies.get(i).setLocation(bodies.get(i).getLocation().add(change));
			bodies.get(i).setVelocity(bodies.get(i).getVelocity().add(bodies.get(i).getAcceleration().mul(60)));
		}
	}
